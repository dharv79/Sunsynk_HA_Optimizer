...
- **Sensors** (`sensor.py`): expose `OptimizerState` fields; `import_plan_end` and `flux2_action` have rich `extra_state_attributes` exposing the full plan/action dicts. Four dedicated adaptive learning sensors read from `last_import_plan`: `forecast_correction`, `overnight_drain_adjustment`, `evening_soc_adjustment`, `effective_charge_rate` — each exposes `days_collected`, `days_required`, and `active` in `extra_state_attributes`. Thresholds are defined in `_ADAPTIVE_THRESHOLDS`. The `consumption` sensor (native value `"{day_load_kwh} kWh today"`) merges `last_morning_state`, `last_day_actuals`, `last_peak_window_usage`, `last_daily_cost`, and `last_year_to_date_cost` into one `extra_state_attributes` dict for the dashboard's Consumption section — field names don't collide (`peak_*`/`year_to_date_*` are distinctly prefixed) so it's a plain merge, except `last_daily_cost`'s own `date` is exposed separately as `daily_cost_date` since it's inherently a day behind the others (see below). `last_day_actuals` exists solely to give this sensor something to read; before v1.0.10b3 the 22:00 day-actuals capture was logged/notified but never persisted to `OptimizerState`.
  ...
  **Octopus Energy cost link (optional, v1.0.11+).** Three free-text config fields — `CONF_OCTOPUS_IMPORT_COST_SENSOR`, `CONF_OCTOPUS_EXPORT_INCOME_SENSOR`, `CONF_OCTOPUS_GAS_COST_SENSOR` — point at the separately-installed Home Assistant Octopus Energy integration's "previous accumulative cost" sensors (blank = disabled, same graceful-degrade shape as `CONF_HOURLY_FORECAST_SENSOR`; see `optimizer._read_octopus_previous_day_cost`). Those sensors reflect a *prior* calendar day and normally settle a few hours after midnight, so `optimizer._async_capture_daily_cost` reads them as part of the existing 06:00 capture (not 22:00, which would be too early) and tags the resulting `daily_cost` record with a **settled date** rather than blindly assuming yesterday. Octopus's own billing settlement can lag multiple days behind (observed around a UK bank holiday) even while the HA integration itself keeps refreshing successfully — the sensor's value changes but still reflects an older day than yesterday. `_read_octopus_previous_day_cost` handles this with a rolling catch-up window (`_OCTOPUS_CATCH_UP_DAYS = 5`): the import sensor's own `last_reset` attribute (the start of the period it's actually reporting) sets the settled date for the read as long as it falls within the last 5 days, falling back to yesterday otherwise — so a late-arriving reading is logged under its real date once Octopus finally delivers it instead of being discarded for good. Export and gas are each independently checked against that same settled date and returned as `None` if their own `last_reset` disagrees, so a value is never logged under the wrong date even when the sensors settle out of step with each other. Per-day dedup in `data_logger` (`_record_exists`) means a date that's already been logged is a silent no-op on a later re-read, so this can safely re-check every morning without double-logging. The same call recomputes a running year-to-date net cost (`OptimizerState.last_year_to_date_cost`) by summing `net_cost_gbp` across the current calendar year's paired days (`data_logger.async_load_paired_days(days=366)`, filtered by year). `net_cost_gbp = import_cost − export_income + gas_cost`, computed in both `_pair_records` and the live capture; it's `None` whenever any of the three inputs is missing, rather than silently treating a missing sensor as zero cost.
