...
- **`data_logger.py`** records decisions and actuals to monthly JSONL files at `{config_dir}/sunsynk_optimizer_data/YYYY-MM.jsonl`. Four record types: `import_plan` (at 01:55), `morning_state` (at 06:00 — SOC and PV power before solar starts), `day_actuals` (at 22:00 — evening SOC, actual solar kWh, plus `day_load_kwh`/`day_grid_import_kwh`/`day_grid_export_kwh` from the SolarSynkV3 daily totals), and `full_charge_day` (weekly scores). The load/grid fields exist so consumption review doesn't have to infer household load from the evening-SOC swing, which conflates load with solar availability — `day_load_kwh` is the real daily load total. They're carried into the paired-day dict by `_pair_records` (read as `None` on pre-1.0.10 history, which predates the fields) but no `compute_*` method consumes them yet — currently informational/for manual review only. Per-day record types are deduplicated at write time (`_write_record` calls `_record_exists` before appending) to prevent double entries on HA restarts that cross a scheduled-event boundary. Provides four analysis methods used by `optimizer.py` to apply adaptive corrections:
